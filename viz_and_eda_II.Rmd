...
Put legend to the bottom (standard from now on):
...
Theme gray is the standard theme if you don't change it:
...
First create two separate data frames then use data in geom() to combine and create one graph with line and scatterplot:
...
Using fct_relevel to reorders name “by hand”:
...
We import the pulse dataset & tidy the data to improve visualization. Some steps that are helpful in retrospect are using pivot_longer to organize the BDI score and visit time variables, and organizing the visit time variable into a factor with an informative ordering.
